...
import os, sys, shutil, traceback, re
...
def analyze():
    ...
    try:
        ...
        return jsonify({"error":"Enter a valid Daily Budget."}),400
...
def download(filename):
    ...
    if not p.exists():
        ...
        return f"File not found: {filename}. Available: {[x.name for x in files]}", 404
    return send_file(str(p), as_attachment=True, download_name=filename)

if __name__ == "__main__":
    print(f"\n  CoE Tool | Template exists: {TEMPLATE_FILE.exists()} | http://127.0.0.1:8501\n")
